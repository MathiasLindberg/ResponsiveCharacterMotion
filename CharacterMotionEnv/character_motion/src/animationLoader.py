...
from bvh import Bvh
import os
from character_motion.src.animation import Animation
from character_motion.src.vectors import *
import pybullet as p
import transformations as transform
import character_motion.src.helpers as helpers
import json
import character_motion.src.environment as environment
import character_motion.src.character as character
...
# Specify animations and their corresponding segments, each segment is defined by a (start frame, end frame) -tuple.
...
def loadAnimations(fromJSON): # Load animation, either from JSON or BVH
...
def __convertAnimations(): # Convert animations from BVH to JSON
    global animationNames
    path = os.path.dirname(os.path.realpath(__file__)) + "/../externals/lafan1"
    (_, _, filenames) = next(os.walk(path))
    for filename in filenames:
...
def __filterAnimation(filepath, startFrame, endFrame): # Get a segment of the specified animation
    res = ""
    frameNum = 0
    with open(filepath) as f:
...
def __filterAnimations(): # Filter all animations stored as JSON files
...
def __constructWorld2CharacterSpaces(): # Construct the character matrices for the stored animations
    args = {"useGUI":False}
